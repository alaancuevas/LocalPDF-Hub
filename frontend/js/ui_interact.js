...
function showView(viewToShow) {
    [viewHome, viewProceso, viewCargando, viewCompletado, viewError, viewAbout].forEach(v => {
        if(v) v.classList.add('hidden');
    });
    
    if (viewToShow === viewCargando) { 
        if(viewProceso) viewProceso.classList.remove('hidden'); 
        if(viewCargando) viewCargando.classList.remove('hidden'); 
    }
    else { 
...
if(btnAbout) {
    btnAbout.addEventListener('click', () => showView(viewAbout));
}

if(btnBackFromAbout) {
    btnBackFromAbout.addEventListener('click', () => showView(viewHome));
}
...
btnToolMerge.addEventListener('click', () => {
    ...
    btnTransform.querySelector('span[data-i18n]').setAttribute('data-i18n', 'btnTransformText');
    btnTransform.querySelector('.material-symbols-outlined').innerText = 'settings_suggest';
